...
export function Carousel(props: PropsWithChildren<CarouselProps>) {
	const { defaultIndex, height, autoplayDelay, delay, animationDelay, autoplay, autoplayReverse, radioAppear, touchDiff, viewportBoxshadow, style, classname, width} = props;
	// 设置需要展示的元素
	...
	useEffect(() => {
		...
		return () => window.clearTimeout(timer);
	}, [autoplayDelay, indexMap, totalLen, autoplay, autoplayReverse]);
