...
export function Progress(props: PropsWithChildren<ProgressProps>) {
	const { count, countNumber, height, circle, size, progressText,
		flashColor, primary, secondary, bottomColor, style, classname, } = props;
	...
	useEffect(() => {
		if (circle) {
			...
			let dashArray = perimeter * percent + " " + (1 - percent)
